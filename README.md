...
git init
git add .
git commit -m "Initial VedaAI app"
git branch -M main
...
- The project is intentionally structured so frontend and backend can be deployed independently.
- The PDF endpoint is authenticated, so the frontend should always fetch it through the logged-in session.
- The current UI is designed as a premium light-glass product surface with warm neutral tones.
- Replace the placeholders in the screenshots section before publishing the repository publicly.
